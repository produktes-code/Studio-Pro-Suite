...
class TestHealthCheck:
    ...
    def test_health_ready_has_portaudio_field(self, mock_sd):
        ...
        response = client.get("/health/ready")
        data = response.json()
        assert "portaudio" in data, \
            f"Falta campo 'portaudio'. Respuesta: {data}"
...
class TestRateLimiting:
    ...
    def test_limiter_configured(self):
        ...
        assert hasattr(app.state, 'limiter'), \
            "La app no tiene limiter en app.state"
...
class TestAppConfiguration:
    ...
    def test_app_version(self):
        ...
        assert app.version == "1.0.0", \
            f"Versión incorrecta: {app.version}"
    ...
    def test_eight_services_instantiated(self):
        ...
        service_modules = [
            'recording', 'mixing', 'mastering', 'effects',
            'sampler', 'synthesizer', 'analysis', 'library'
        ]
