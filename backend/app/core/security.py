...
async def validate_uploaded_file(file: UploadFile) -> str:
    ...
    if content_length:
        ...
        if size > settings.MAX_UPLOAD_SIZE:
            logger.error(
                f"File size ({size} bytes) exceeds limit of {settings.MAX_UPLOAD_SIZE} bytes"
            )
            raise HTTPException(
    ...
    if actual_size > settings.MAX_UPLOAD_SIZE:
        logger.error(
            f"Actual file size ({actual_size} bytes) exceeds limit of {settings.MAX_UPLOAD_SIZE} bytes"
        )
        raise HTTPException(
            status_code=status.HTTP_413_REQUEST_ENTITY_TOO_LARGE,
            detail="File size exceeds the maximum allowed limit of 2GB.",
