...
class RecordingService:
    ...
    def record_microphone(self, duration_seconds: float, sample_rate: int = 44100) -> str:
        ...
        try:
            ...
            logger.warning(f"Error querying audio devices: {e}. Falling back to simulation.")

        if has_input:
            try:
                logger.info(f"Recording real microphone for {duration_seconds}s at {sample_rate}Hz...")
                recording = sd.rec(
                ...
                logger.error(f"Failed to record with sounddevice: {e}. Falling back to simulation.")

        # Fallback simulation (e.g. in headless environments, CI/CD, or servers without microphone)
        logger.info(f"Simulating microphone recording (generating a 440Hz sine wave + ambient noise)...")
