...
class EffectsService:
    ...
    def apply_lowpass_filter(self, samples: np.ndarray, cutoff_hz: float, sr: int) -> np.ndarray:
        nyq = sr / 2.0
        cutoff = min(0.99, max(0.01, cutoff_hz / nyq))
        b, a = signal.butter(2, cutoff, btype='lowpass')
        return signal.lfilter(b, a, samples, axis=0)

    def apply_highpass_filter(self, samples: np.ndarray, cutoff_hz: float, sr: int) -> np.ndarray:
        nyq = sr / 2.0
        cutoff = min(0.99, max(0.01, cutoff_hz / nyq))
        b, a = signal.butter(2, cutoff, btype='highpass')
        return signal.lfilter(b, a, samples, axis=0)

    def apply_delay(self, samples: np.ndarray, delay_time_sec: float, feedback: float, wet: float, sr: int) -> np.ndarray:
    ...
    def apply_reverb(self, samples: np.ndarray, room_size: float, wet: float, sr: int) -> np.ndarray:
        """
    ...
    def apply_chorus(self, samples: np.ndarray, depth_ms: float = 3.0, rate_hz: float = 1.5, sr: int = 44100) -> np.ndarray:
        """
        ...
        if len(samples.shape) == 2:
            ...
            for c in range(channels):
                ...
                for i in range(n_samples):
                    ...
                    mod_delay = base_delay_samples + offset + int(max_mod_samples * lfo[i])
                    src_idx = i - mod_delay
